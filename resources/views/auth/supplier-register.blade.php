    ...
    <div class="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 relative">
        ...
        <div class="w-full max-w-2xl bg-white border border-slate-200 rounded-2xl shadow-lg p-6 sm:p-8 space-y-6 mx-4 sm:mx-auto relative z-10"
            ...
            <div class="text-center space-y-1">
                ...
                <p class="text-sm text-slate-500">Daftarkan perusahaan Anda dan tunggu persetujuan admin.</p>
            </div>
            ...
            <form method="POST" action="{{ route('supplier.register') }}" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="space-y-1">
                        ...
                        <x-text-input id="company_name" class="block w-full rounded-xl border-slate-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm py-2.5" type="text" name="company_name" :value="old('company_name')" required autofocus placeholder="PT. Contoh Sejahtera" />
                        <x-input-error :messages="$errors->get('company_name')" />
                    </div>

                    <div class="space-y-1">
                        <x-input-label for="supplier_name" value="Nama Kontak (PIC)" />
                        <x-text-input id="supplier_name" class="block w-full rounded-xl border-slate-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm py-2.5" type="text" name="supplier_name" :value="old('supplier_name')" required placeholder="Budi Santoso" />
                        <x-input-error :messages="$errors->get('supplier_name')" />
                    </div>

                    <div class="space-y-1">
                        <x-input-label for="email" value="Email Perusahaan" />
                        <x-text-input id="email" class="block w-full rounded-xl border-slate-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm py-2.5" type="email" name="email" :value="old('email')" required placeholder="info@perusahaan.com" />
                        <x-input-error :messages="$errors->get('email')" />
                    </div>

                    <div class="space-y-1">
                        <x-input-label for="phone" value="Nomor Telepon / WA" />
                        <x-text-input id="phone" class="block w-full rounded-xl border-slate-300 focus:border-teal-500 focus:ring-teal-500 shadow-sm py-2.5" type="text" name="phone" :value="old('phone')" required placeholder="081234567890" />
                ...
                <div class="pt-4 border-t border-slate-100 flex flex-col gap-4">
                    <div class="bg-slate-50 p-3 rounded-lg flex items-start gap-2">
                        ...
                        <p class="text-xs text-slate-500 leading-relaxed">
                            Akun Anda akan berstatus <strong>Pending</strong> setelah pendaftaran. Anda baru dapat melakukan stok barang setelah disetujui oleh Admin Gudang.
                        </p>
                    </div>

                    <div class="flex flex-col-reverse sm:flex-row items-center gap-3 justify-end">
                        <a href="{{ route('login') }}"
                           class="w-full sm:w-auto inline-flex items-center justify-center h-10 px-5 rounded-lg 
                        ...
                        <x-primary-button class="w-full sm:w-auto justify-center h-10 px-6 bg-teal-600 hover:bg-teal-700">
                            Daftar Sekarang
                        </x-primary-button>
                    </div>
                </div>
