<div x-show="isScanOpen" x-cloak
    ...
    <div class="w-full max-w-sm bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col relative"
        ...
        <div class="px-5 py-4 flex justify-between items-center bg-white border-b border-slate-50">
            <div>
                ...
                <p class="text-xs text-slate-500"
                    x-text="scanMode === 'search' ? 'Mode: Cari Produk' : (scanMode === 'incoming' ? 'Mode: Barang Masuk' : 'Mode: Barang Keluar')">
                </p>
            </div>
        ...
        <div
            class="relative bg-black w-full aspect-[4/5] sm:aspect-square flex items-center justify-center overflow-hidden group">

            <div id="reader" class="w-full h-full object-cover"></div>

            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none"
                x-show="!isFileScanning">
                <div class="w-64 h-64 relative border border-white/30 rounded-3xl">
                    <div
                        class="absolute top-0 left-0 w-6 h-6 border-t-[3px] border-l-[3px] border-white rounded-tl-2xl">
                    </div>
                    <div
                        class="absolute top-0 right-0 w-6 h-6 border-t-[3px] border-r-[3px] border-white rounded-tr-2xl">
                    </div>
                    <div
                        class="absolute bottom-0 left-0 w-6 h-6 border-b-[3px] border-l-[3px] border-white rounded-bl-2xl">
                    </div>
                    <div
                        class="absolute bottom-0 right-0 w-6 h-6 border-b-[3px] border-r-[3px] border-white rounded-br-2xl">
                    </div>
                </div>
            ...
            <button type="button" x-show="cameras.length > 1" @click="
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-teal-500/90 z-40 backdrop-blur-sm transition-opacity"
                x-show="dragOver" x-transition:enter="duration-200 ease-out" x-transition:enter-start="opacity-0"
        ...
        <div class="p-4 bg-white border-t border-slate-50">
            <label for="file-upload"
                class="flex items-center justify-center w-full py-3 gap-2.5 rounded-xl border border-slate-200 bg-slate-50 text-slate-600 font-medium text-sm hover:bg-slate-100 hover:border-slate-300 active:scale-[0.98] transition cursor-pointer">
