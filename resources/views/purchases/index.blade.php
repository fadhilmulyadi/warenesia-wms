...
@section('content')
    <div class="max-w-6xl mx-auto space-y-4 text-xs">
        ...
        <form method="GET" action="{{ route('purchases.index') }}" class="flex flex-wrap items-center gap-2">
            <input
                type="text"
                name="q"
                value="{{ $search }}"
            ...
            @can('export', \App\Models\IncomingTransaction::class)
                <a
                    href="{{ route('purchases.export', request()->query()) }}"
                            ...
                            @can('viewAny', \App\Models\IncomingTransaction::class)
                                <td class="px-4 py-2 text-right">
                                    @can('view', $transaction)
                                        <a
                                            href="{{ route('purchases.show', $transaction) }}"
