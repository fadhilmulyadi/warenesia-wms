...
@section('content')
    @php
        $canViewProductDetail = $product
            ? auth()->user()?->can('view', $product)
            : auth()->user()?->can('viewAny', \App\Models\Product::class);
        $canCreatePurchase = auth()->user()?->can('create', \App\Models\IncomingTransaction::class);
        $canCreateSale = auth()->user()?->can('create', \App\Models\OutgoingTransaction::class);
    @endphp
    ...
    >
        <div class="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
            <form
                method="POST"
                action="{{ route('barcode.scan.handle') }}"
                class="space-y-2"
            >
                @csrf
                    ...
                    <div class="flex-1 space-y-1">
                        ...
                        <div class="flex flex-col gap-1 w-full">
                            ...
                            @can('create', \App\Models\IncomingTransaction::class)
                                <a
                                    href="{{ route('purchases.create', ['product_id' => $product->id]) }}"
                            ...
                            @can('create', \App\Models\OutgoingTransaction::class)
                                <a
                                    href="{{ route('sales.create', ['product_id' => $product->id]) }}"
